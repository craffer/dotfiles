# Generated by Powerlevel10k configuration wizard on 2020-02-13 at 15:29 EST.
...
() {
  ...
  function my_git_formatter() {
    ...
    if [[ $VCS_STATUS_NUM_CONFLICTED > 0 ]]; then
      branch_color=${conflicted}
    elif [[ $VCS_STATUS_NUM_STAGED > 0 || $VCS_STATUS_NUM_UNSTAGED > 0 ]]; then
      branch_color=${modified}
    elif [[ $VCS_STATUS_NUM_UNTRACKED > 0 ]]; then
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      res+="${branch_color}${POWERLEVEL9K_VCS_BRANCH_ICON}"
    ...
    [[ -z $where ]] && res+="${meta}@${branch_color}${VCS_STATUS_COMMIT[1,8]}"

    # Show tracking branch name if it differs from local branch.
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${meta}:${branch_color}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"  # escape %
    fi
    ...
    # 42* (in pink) if have stashes.
    (( VCS_STATUS_STASHES        )) && res+=" ${pink}${VCS_STATUS_STASHES}*"
    # 42⇣ if behind the remote.
    (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${gray}${VCS_STATUS_COMMITS_BEHIND}⇣"
    # 42⇡ if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
    (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${gray}${VCS_STATUS_COMMITS_AHEAD}⇡"
    # 42⇠ if behind the push remote.
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" ${gray}${VCS_STATUS_PUSH_COMMITS_BEHIND}⇠"
    (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" "
    # 42⇢ if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && res+="${gray}${VCS_STATUS_PUSH_COMMITS_AHEAD}⇢"
  ...
  typeset -g POWERLEVEL9K_VCS_DISABLED_WORKDIR_PATTERN='~/blt'
  ...
  # Custom prefix.
  typeset -g POWERLEVEL9K_VCS_PREFIX='%249Fon '
  ...
  typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND=138
